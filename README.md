...
├── package.json                      # Monorepo developer scripts (test, build, docker)
├── .env.example                      # Root environment configuration template
├── .gitignore                        # Standard exclusions (secrets, binaries, node_modules)
├── README.md                         # Portfolio documentation & architectural guide
└── FraudDetectionDashboard/
    ├── docker-compose.yml            # Multi-container orchestration (Mongo, Node, Nginx)
    ├── backend/
    │   ├── package.json              # Express, Mongoose, Socket.IO dependencies
    │   ├── Dockerfile                # Multi-stage Linux Docker build (g++ + Node.js)
    │   ├── .env.example              # Backend environment template
    │   ├── src/
    │   │   ├── server.js             # HTTP/WebSocket server, Helmet, rate-limit, health
    │   │   ├── config/
    │   │   │   ├── database.js       # Resilient Mongoose connection with fallback
    │   │   │   └── engine.js         # Cross-platform C++ binary locator
    │   │   ├── models/               # Mongoose schemas: Transaction, FraudAlert, FraudNetwork
    │   │   ├── controllers/          # Request handlers for transactions, alerts, analytics
    │   │   ├── routes/               # Express route declarations
    │   │   └── services/
    │   │       ├── cppEngineService.js # C++ --json orchestrator & bulk upsert persistence
    │   │       └── streamManager.js    # C++ --stream subprocess lifecycle manager
    │   └── tests/
    │       ├── verify_phase8.js      # 20-point automated API and resilience test suite
    │       └── verify_db_persistence.js # In-memory MongoDB persistence and deduplication test
    ├── frontend/
    │   ├── package.json              # React 18, Vite, Recharts, Lucide dependencies
    │   ├── Dockerfile                # Multi-stage build (Vite compilation + Nginx)
    │   ├── nginx.conf                # Reverse proxy for React SPA, /api, and WebSockets
    │   ├── .env.example              # Frontend environment template
    │   └── src/
    │       ├── App.jsx               # Router configuration and global alert toasts
    │       ├── pages/
    │       │   ├── Dashboard.jsx     # Executive Overview, KPIs, Charts, Suspicious Accounts
    │       │   ├── Monitoring.jsx    # Live Operations console, TPM meter, raw telemetry
    │       │   ├── Transactions.jsx  # Active transaction ledger
    │       │   ├── Alerts.jsx        # Prioritized incident feed
    │       │   ├── Networks.jsx      # SVG topological directed fraud network visualizer
    │       │   └── History.jsx       # Historical archive with date and risk filters
    │       └── services/
    │           ├── api.js            # Axios REST client with multi-attribute query support
    │           └── socket.js         # Singleton Socket.IO connection manager
    └── fraud-engine/
        ├── CMakeLists.txt            # Portable CMake build configuration (C++17 + pthreads)
        ├── include/                  # Header definitions (Transaction, Graph, DSU, etc.)
        └── src/
            ├── main.cpp              # Entrypoint (--json, --stream, and verification modes)
            ├── TransactionGenerator.cpp # Synthetic transaction generator
            ├── FraudInjector.cpp     # Scenario injection (high value, bursts, rings)
            ├── FraudDetector.cpp     # Orchestrator for graph and velocity detectors
            ├── TransactionGraph.cpp  # Directed graph, BFS, and DFS cycle detection
            ├── DSU.cpp               # Disjoint Set Union with path compression
            ├── RiskScorer.cpp        # Multi-factor rule-based scoring engine
            ├── JsonSerializer.cpp    # High-throughput RFC 8259 JSON serialization
            └── StreamSimulator.cpp   # Real-time transaction streaming simulator
```
